...
| ------------ | ------------------------ | ----- |
| Hammer       | *Compatible thru Wine*   | Needs to be run with `-winecompat`, see [Running Windows Tools on Linux](./tools_on_linux). |
| HLMV         | *Compatible thru Wine* | Same with Hammer. |
...
| SDK launcher | Native | Can be launched from Steam or from `/bin/linux64` |
| Workshop GUI | Native | Must be launched from SDKLauncher executable in `/bin/linux64` to publish addons |
