...
If the script fails (i.e. syntax error in the original captions), it will show the line and column where the error occured.

> [!NOTE]
> The converter only parses `.txt` files, it is not possible to convert `.dat` using it

![Converter drag-n-drop support](converter.png)
