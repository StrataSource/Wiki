...
As an example, let's assume the `object` class implements opAssign (assignment operator method), doing:

```cpp
type my_object = 10;
```

Will result in the previously explained behavior, which is **not recommended**, however, doing:
...
There is no need to separately initialize my_type to a variable (`my_type my_obj(...);`).

## Handles and Functions

Handles can be passed as arguments for function and they can be returned.
